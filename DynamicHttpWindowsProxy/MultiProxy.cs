...
internal struct MultiProxy
{
    ...
    {
        Debug.Assert(_uris != null || _proxyConfig != null,
        ...
        {
            bool hasProxy = TryParseProxyConfigPart(_proxyConfig.AsSpan(_currentIndex), _secure, out uri!,
    ...
    /// </remarks>
    private static bool TryParseProxyConfigPart(ReadOnlySpan<char> proxyString, bool secure,
        [NotNullWhen(true)] out Uri? uri, out int charactersConsumed)
    {
        ...
        while (true)
        {
            ...
            if ((proxyType & wantedFlag) != 0 && Uri.TryCreate(string.Concat("http://", proxyString.Slice(0, iter)),
